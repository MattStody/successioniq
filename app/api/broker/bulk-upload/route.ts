...
Location: ${row.region}, ${row.country}
...
Revenue Trend: ${row.revenue_trend ?? "not specified"}
What's Included: ${row.whats_included}`,
        },
      ],
    });
    ...
    return `This established ${row.industry} business presents a compelling acquisition opportunity. With $${Number(row.annual_revenue).toLocaleString()} in annual revenue and ${row.years_operating} years of operating history, it represents a stable and proven enterprise. The business includes ${row.whats_included} and offers a ${String(row.transition_period ?? "").replace(/_/g, " ")} transition period to ensure continuity.`;
  }
}
